# 🗂️ Projects Portfolio

A curated collection of front-end projects — including personal work and challenges from Frontend Mentor — showcasing skills in HTML, CSS, and JavaScript.

---

## 🚀 Projects

### Personal

| Project | Description | Live | Code |
|---|---|---|---|
| Savelitz | Landing page for a YouTube download web app built with Node.js/Express | — | [Code](https://github.com/Gustavdoiss/Projects-Portifolium/tree/main/Projects/Savelitz) |

### Frontend Mentor Challenges

| Challenge | Techs | Live | Code |
|---|---|---|---|
| Typing Speed Test | Fetch API · CSS (Flexbox) · localStorage · Custom Properties | [Preview](https://gustavdoiss.github.io/Projects-Portifolium/Projects/Frontend-Mentor/typing-speed-test-main/index.html) | [Code](https://github.com/Gustavdoiss/Projects-Portifolium/tree/main/Projects/Frontend-Mentor/typing-speed-test-main) |
| Browser Extension Manager UI | Fetch API · CSS (Flexbox + Grid) · HTML5 Semantic · Custom Properties | [Preview](https://gustavdoiss.github.io/Projects-Portifolium/Projects/Frontend-Mentor/browser-extensions-manager-ui-main/index.html) | [Code](https://github.com/Gustavdoiss/Projects-Portifolium/tree/main/Projects/Frontend-Mentor/browser-extensions-manager-ui-main) |
| FAQ Accordion | HTML · CSS (Flexbox) · JS | [Preview](https://gustavdoiss.github.io/Projects-Portifolium/Projects/Frontend-Mentor/faq-accordion-main/index.html) | [Code](https://github.com/Gustavdoiss/Projects-Portifolium/tree/main/Projects/Frontend-Mentor/faq-accordion-main) |

---

## 🛠️ Tech Stack

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)

---

## 👤 Author

- GitHub: [@Gustavdoiss](https://github.com/Gustavdoiss)
- LinkedIn: [Gustavo de Sena](https://linkedin.com/in/gustavo-de-sena)
